...
for k=1:N_rounds
    ...
    for j=1:N_it-N
        ...
        if exitf(j)<=0
            fprintf("failed")
            [u_n,~,~,exitf_i,~,~]=myfmincon(FullStateUpdate,[u_n(1:end-1);0],[],[],C,d,0*N,8*N,myoptions);
            if exitf_i<=0
                failed=failed+1;
            end
        end
        ...
        %Every 10% prints a loading message
        if rem(j,ceil(N_it/10))==0
            eltime = eltime+toc;
            int = ceil(N_it/10);
            fprintf("We are at: %d %% of the round %d \n In total it takes: %f s \n On average it takes: %f s ad iterazione \n",round(j/(N_it)*100),k,toc,eltime/int)
            eltime = 0;
            tic
        end
...
%u=0
try
fprintf("The first attempt failed %d volte, it also faiuled with u=0 %d times",sum(exitf<=0),failed)
end

%Extracts the total vectors
tot_speed=driving_cycle(1,start:end);
tot_acceleration=driving_cycle(2,start:end);
tot_gear=driving_cycle(3,start:end);
tot_dislivello=driving_cycle(4,start:end);
