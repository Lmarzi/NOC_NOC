...
for i=1:27
    ...
    tot_speed=[tot_speed,driving_cycle(1,:)];
    tot_acceleration=[tot_acceleration,driving_cycle(2,:)];
    tot_gear=[tot_gear,driving_cycle(3,:)];
    tot_dislivello=[tot_dislivello,driving_cycle(4,:)];
end
%driving_cycle=WLTC;%[tot_speed;tot_acceleration;tot_gear;tot_dislivello];

for window=1:10   
    start=0;%Definisci l'istante iniziale
    N=window; %Time horizon ->compare sol with 5
    SOC_START=0.52;
    N_rounds=1;
    ...
    for k=1:N_rounds
        ...
        %fprintf("È stato scelto il ciclo %s, il SOC è %f\n",name, SOC(end))
         ...
         tot_dislivello=[tot_dislivello,zeros(length(driving_cycle),1)];%driving_cycle(4,1:end-N)];
        ...
        for j=1:N_it-N
            ...
            dislivello=zeros(N,1);%driving_cycle(4,start+j:start+N-1+j);
    ...
    % end
    % 
     StateUpdate=@(input,cur_SOC,i)my_hev(tot_speed(i),tot_acceleration(i),tot_gear(i),0,cur_SOC,input,etam_int,etaeng_int,Tmmax_int,Tmmin_int,Temax_int);
     FullStateUpdate=@(u)full_horizon(u,SOC_START,StateUpdate);
    
    [cost,SOC2plot,T_req,T_giv,mf,I_c,V_c]=FullStateUpdate(u);
    tot_mf_WLTC(window)=sum(mf)*1000
    %u_back{window}=u;

    %fprintf("Terminato il ciclo con finestra %f, il mf è: %f\n",window,tot_mf(window))
